...
_velocities = ti.Vector.field(2, dtype=ti.f32, shape=(res, res))
_new_velocities = ti.Vector.field(2, dtype=ti.f32, shape=(res, res))
velocity_divs = ti.field(dtype=ti.f32, shape=(res, res))
velocity_curl = ti.field(dtype=ti.f32, shape=(res, res))
_pressures = ti.field(dtype=ti.f32, shape=(res, res))
_new_pressures = ti.field(dtype=ti.f32, shape=(res, res))
_density_buffer = ti.field(dtype=ti.f32, shape=(res, res))
_new_density_buffer = ti.field(dtype=ti.f32, shape=(res, res))
color_buffer = ti.Vector.field(3, dtype=ti.f32, shape=(res, res))
...
def step(mouse_data):
    ...
    advect(velocities_pair.cur, density_pair.cur, density_pair.nxt)
    velocities_pair.swap()
    density_pair.swap()

    # add density to density field
    add_density(int(res * 0.25), 50, 25, 0.8)
